...
namespace ContentWarningArchipelago.Core
{
    public class ArchipelagoClient
    {
        ...
        /// <summary>True once the ClientGoal win packet has been sent this session.
        /// Prevents duplicate sends if CheckWinCondition() is called repeatedly.</summary>
        ...
        public void ActivateCheck(long locationId)
        {
            ...
            // Re-evaluate the win condition after every new check.
            // CheckWinCondition() is idempotent — _goalSent guards against double-sends.
            CheckWinCondition();
        }
        ...
        /// <summary>
        /// Evaluates all active goals against the locally-tracked game state and
        /// sends the <c>ClientGoal</c> win packet if every active goal is satisfied.
        ///
        /// <para>
        /// Called automatically from <see cref="ActivateCheck"/> after each location
        /// check fires, so the win packet is sent as soon as the last required check
        /// lands.  The <c>_goalSent</c> flag ensures the packet is sent exactly once
        /// per session, even though this method may be called many times.
        /// </para>
        ///
        /// <para>
        /// Each goal block is only evaluated when its matching toggle in
        /// <see cref="APSaveData"/> is <c>true</c>.  Goals that are disabled in the
        /// player's YAML are silently skipped and do not block the win condition.
        /// </para>
        /// </summary>
        public void CheckWinCondition()
        {
            ...
            bool anyGoalActive = s.viralSensationGoal || s.viewsGoal || s.quotaGoal
                                 || s.monsterHunterGoal || s.hatCollectorGoal;
            if (!anyGoalActive)
            {
                Plugin.Logger.LogWarning(
                    "[AP] CheckWinCondition: no goals are active in slot data — skipping.");
                return;
            }

            // ---- Viral Sensation ------------------------------------------------
            // Satisfied when the "Viral Sensation Achieved" location has been sent
            // (the client fires it once currentQuotaViews crosses 1,000,000).
            if (s.viralSensationGoal)
            {
                long locId = LocationData.GetId(LocationNames.ViralSensationAchieved);
                if (locId < 0 || !APSave.IsLocationChecked(locId))
                {
                    Plugin.Logger.LogDebug(
                        "[AP] CheckWinCondition: Viral Sensation not yet achieved.");
                    return;
                }
            }
            ...
            // ---- Quota Goal -----------------------------------------------------
            // Satisfied when the Nth quota-completion check has been sent, where N
            // equals quotaCount from slot data.
            if (s.quotaGoal)
            {
                long locId = LocationData.GetId(LocationNames.MetQuotaPrefix + s.quotaCount);
                if (locId < 0 || !APSave.IsLocationChecked(locId))
                {
                    Plugin.Logger.LogDebug(
                        $"[AP] CheckWinCondition: Quota Goal not yet reached " +
                        $"(need quota {s.quotaCount}).");
                    return;
                }
            }

            // ---- Monster Hunter -------------------------------------------------
            // Base monster filming locations occupy offsets 300–329 in LocationData
            // (30 total, one per distinct monster species).  Count how many have
            // been checked and compare against monsterHunterCount from slot data.
            if (s.monsterHunterGoal)
            {
                int checkedMonsters = 0;
                for (int offset = 300; offset <= 329; offset++)
                {
                    if (APSave.IsLocationChecked(LocationData.BaseId + offset))
                        checkedMonsters++;
                }

                if (checkedMonsters < s.monsterHunterCount)
                {
                    Plugin.Logger.LogDebug(
                        $"[AP] CheckWinCondition: Monster Hunter not yet reached " +
                        $"({checkedMonsters}/{s.monsterHunterCount} monsters filmed).");
                    return;
                }
            }

            // ---- Hat Collector --------------------------------------------------
            // Hat purchase locations occupy offsets 600–630 in LocationData
            // (31 total, one per hat).  Count how many have been checked and
            // compare against hatCollectorCount from slot data.
            if (s.hatCollectorGoal)
            {
                int checkedHats = 0;
                for (int offset = 600; offset <= 630; offset++)
                {
                    if (APSave.IsLocationChecked(LocationData.BaseId + offset))
                        checkedHats++;
                }

                if (checkedHats < s.hatCollectorCount)
                {
                    Plugin.Logger.LogDebug(
                        $"[AP] CheckWinCondition: Hat Collector not yet reached " +
                        $"({checkedHats}/{s.hatCollectorCount} hats purchased).");
                    return;
                }
            }

            // ---- All active goals satisfied — send the win packet ---------------
            Plugin.Logger.LogInfo(
                "[AP] All active goals complete — sending ClientGoal win packet!");
            _goalSent = true;
            SendCompletion();
        }
        ...
        /// Sends the <c>ClientGoal</c> status update packet.
        /// </summary>
